...
def encode_request(args, system_mode=None, dialog_logger=None):
    ...
    if dialog_logger is None:
        ...
        compulsory_output_function = output_function
    else:
    ...
    # input request
    request_queue = getattr(args, 'request_queue', None)
    if request_queue is not None and len(request_queue) > 0:
        input_text = request_queue.pop(0)
        compulsory_output_function('Using queued request input.')
    elif request_queue is not None and getattr(args, 'auto_end_on_exhausted',
                                                False):
        input_text = getattr(args, 'fallback_request_text',
                             "That's all, thank you.")
        compulsory_output_function(
            'Request queue exhausted; using fallback end request.')
    else:
        compulsory_output_function(
