...
def dialog_with_real_user(field_model,
    ...
    with torch.no_grad():
        start_image, start_label, start_score = \
            field_model.synthesize_and_predict(torch.from_numpy(latent_code).to(device)) # noqa
    ...
    while True:
        if max_rounds is not None and round_idx >= max_rounds:
            dialog_logger.info(
                f'Max dialog rounds ({max_rounds}) reached. Ending dialog.')
            state_log.append('end')
            text_log.append('SYSTEM: Dialog ended due to max round limit.')
            break
